...
export async function POST(request: Request) {
  try {
    ...
    const fileName = `network-${Date.now()}-${randomBytes(6).toString("hex")}.${extension}`;
    const uploadDir = path.join(process.cwd(), "public", "uploads", "networks");
    await fs.mkdir(uploadDir, { recursive: true });
    await fs.writeFile(path.join(uploadDir, fileName), buffer);

    return NextResponse.json({ url: `/uploads/networks/${fileName}` });
  } catch (error) {
    console.error("Network logo upload error:", error);
